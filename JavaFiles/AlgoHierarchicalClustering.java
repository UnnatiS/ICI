...
import reqclustering.CommonDataSharing;
...
public class AlgoHierarchicalClustering {
	...
	List<ClustClass> clstData = new ArrayList<ClustClass>();
...
//				System.out.println("val");
			...
			cluster.reqIdsinCluster.add(count);
			ClustClass cl = new ClustClass(count++);
			clstData.add(cl);
		...
		do {
			...
			int cnt = 0;
			for (ClusterWithMean clm : clusters) {
				// System.out.println("Cluster with mean : " +clm.getmean() + " contains :
				// "+clm.reqIdsinCluster.toString()+" requirements");
				System.out.println("cluster :" + cnt++);
				for (int i : clm.reqIdsinCluster) {
					System.out.println(i + " : " + CommonDataSharing.originalRequirements.get(i));
				}
			}
			System.out.println("To stop the iteration enter 'y' oterwise 'n': ");
		...
		saveToFile("d://agloOutClustering.txt");
		// return the clusters
		return clusters;
	}
	...
	private boolean mergeTheClosestCluster() {
		...
		List<List<ClustClass>> mergedListclass = new ArrayList<List<ClustClass>>();
		...
		for (int i = 0; i < clusters.size(); i++) {
			for (int j = i + 1; j < clusters.size(); j++) {
				...
				if (distance < minClusterDistance && distance <= maxDistance) {
					...
					lst.add(j);
					mergedList.add(lst);
					clusterToMerge1.reqIdsinCluster.addAll(clusters.get(i).reqIdsinCluster);
					clusterToMerge2.reqIdsinCluster.addAll(clusters.get(j).reqIdsinCluster);
				}
			}
		}
		...
		System.out.println("The Iteration Number: " + (iterationCount + 1));
		// outputData += "The Iteration Number: "+(iterationCount+1);
		for (int i = 0; i < mergedList.size(); i++) {
			System.out.println(";" + mergedList.get(i).get(0) + ":" + mergedList.get(i).get(1));
			// outputData += CommonDataSharing.requirements.get(mergedList.get(i).get(0));
		}
		...
		for (DoubleArray vector : clusterToMerge2.getVectors()) {
			...
			clusterToMerge1.reqIdsinCluster.addAll(clusterToMerge2.reqIdsinCluster);
		}
	...
	public void showClusters() {
		...
		for (int i = 0; i < clusters.size(); i++) {
			...
			if (clusters.get(i).getVectors().size() >= 1) {
				// write the cluster
				System.out.print(clusters.get(i).toString());
				// if not the last cluster, add a line return
				if (i < clusters.size() - 1) {
					System.out.println();
				}
